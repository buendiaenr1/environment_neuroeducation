...
barplot(dtm_d[1:150,]$freq, las = 3, names.arg = dtm_d[1:150,]$word,
        col ="lightgreen", main ="Top 150 most frequent words",
...
# Find associations 
#terms = c("illness","disease","sickness","disorder","ailment","complaint","epidemic","provoke","trigger","induce","elicit","drive","promote","draw","fuel","needle","symptom","acumm","conne","depend","reference","relativa","cogni","relat","link","contribu","neuro","pedag","didac","lear","teach")
terms = c("ailment","complaint","drive","draw","fuel","relat","neuro","teach")
...
crude <- readLines("enfer_f.csv")   #gl_enfer.csv
...
acq <- readLines("enfer_f.csv")   #enfer_f.csv o usar acq <- dat2
...
terms = c("ailment","complaint","drive","draw","fuel","relat","neuro","teach")
assocs <- findAssocs(dtm, term=terms, corlimit=0.49)
...
plot(dtm, term=freq.terms, corThreshold=0.7, weighting=T, 
  edgeAttrs=eAttrs)

################################################################################### ++++  este es el usado en art +++++ ###########################
...
acq <- readLines("enfer_f.csv") 
dtm <- DocumentTermMatrix(acq,
  control = list(weighting = function(x) weightTfIdf(x, normalize=FALSE),
  stopwords = TRUE))
freq.terms <- findFreqTerms(dtm, lowfreq=1)[1:25]
terms = c("ailment","complaint","drive","draw","fuel","relat","neuro","teach")
assocs <- findAssocs(dtm, term=terms, corlimit=0.49)
...
######### a veces funciona y aveces no, lo use cuando funciono .... en articulo
...
library("syuzhet")
library("ggplot2")
text <- readLines(file.choose()) # enferF.csv
...
######################### para el caso de sentimientos de mexico usar
...
#### vector de doc_id correspondientes a la necesidad ambiental
amb<-c(19, 53, 57, 73, 88, 93, 127, 133, 211, 212, 244, 268, 303, 339, 354, 487, 65, 227, 290,471,485)
amb<-sort(unique(amb)) ### sin repetidos y ordenados

x<-text[amb]



write.csv(x,"enfer_mx.csv",row.names=FALSE)
##

text <- readLines("enfer_mx.csv")
# regular sentiment score using get_sentiment() function and method of your choice
# please note that different methods may have different scales
# Other methods include “bing”, “afinn”, “nrc”, and “stanford”.
syuzhet_vector <- get_sentiment(text, method="syuzhet")
...
barplot(
  ...
  cex.names = 0.7, 
  las = 1, 
  main = "Emotions in México", xlab="Percentage"
)
